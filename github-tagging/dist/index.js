...
const file_command_1 = __nccwpck_require__(266);
const utils_1 = __nccwpck_require__(861);
const os = __importStar(__nccwpck_require__(37));
...
(() => {
const fetch = __nccwpck_require__(372);
...
const createTag = async () => {
    ...
    try {
        // get existing tags
        const tagsResponse = await fetch(`${process.env.GITHUB_API_URL}/repos/${process.env.GITHUB_REPOSITORY}/tags`, { headers });
        ...
        const commitMessages = (commitSinceTagData || []).commits.map((item) => `* ${item.commit.message} ${item.html_url} - @${item.author.login}`);
        ...
        const updatedTagVersionNumber = cleanTag === newTag ? Number(tagVersionNumber) + 1 : 1;
